...
celltype_colors <- met.brewer("Signac", 
                              n = length(unique(seurat_data$RNA_celltype)))

names(celltype_colors) <- unique(seurat_data$RNA_celltype)
...
goi <- c("HNF1A", "HNF1B", "NEUROG3", "CHGA", "GCK", "NKX6-1")
...
print(plotDimRed(seurat_data, col_by = "RNA_celltype",
                 color = celltype_colors, plot_type = "rna.umap"))
